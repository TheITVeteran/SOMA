...
import { FileSystemNode, GraphViewMode } from '../types';
...
interface KnowledgeGraphProps {
  ...
  onContextMenu?: (e: React.MouseEvent | any, node: FileSystemNode) => void;
...
const getNodeColor = (node: FileSystemNode, viewMode: GraphViewMode, isRelevant: boolean) => {
    if (isRelevant) {
...
const KnowledgeGraph: React.FC<KnowledgeGraphProps> = ({ 
  ...
  const [hoveredNode, setHoveredNode] = useState<FileSystemNode | null>(null);
  ...
  useEffect(() => {
    ...
    svg.call(zoom as any)
       .call(zoom.transform as any, d3.zoomIdentity.translate(width/2, height/2).scale(0.6));
    ...
    const simulation = d3.forceSimulation(displayNodes as any)
      .force("link", d3.forceLink(links).id((d: any) => d.id).distance(d => (d.target.kind === 'directory' ? 80 : 40)))
      .force("charge", d3.forceManyBody().strength(-300))
      .force("collide", d3.forceCollide().radius((d: any) => d.kind === 'directory' ? 30 : 15).iterations(2))
    ...
    const link = g.append("g")
      ...
      .attr("stroke-width", 1)
      .attr("class", "link")
      .attr("stroke-dasharray", (d: any) => (d.source.isVirtual || d.target.isVirtual) ? "4 4" : "none");
    ...
    node.append("circle")
      .attr("r", (d: any) => d.kind === 'directory' ? 12 : 6)
      .attr("fill", (d: any) => getNodeColor(d, viewMode, relevantNodeIds.includes(d.id)))
      .attr("filter", (d: any) => d.isVirtual || d.lifecycleStatus === 'dead' ? "none" : "url(#glow)") 
      .attr("stroke", (d: any) => d.isVirtual ? "#a855f7" : "white")
      .attr("stroke-width", (d: any) => relevantNodeIds.includes(d.id) ? 2 : (d.isVirtual ? 1 : 0))
      .attr("stroke-dasharray", (d: any) => d.isVirtual ? "2 2" : "none")
      .attr("opacity", (d: any) => d.lifecycleStatus === 'dead' && viewMode === 'lifecycle' ? 0.3 : 0.8)
      .transition().duration(500)
      .attr("fill", (d: any) => getNodeColor(d, viewMode, relevantNodeIds.includes(d.id))); 

    node.append("circle")
      .attr("r", (d: any) => d.kind === 'directory' ? 4 : 2)
      .attr("fill", (d: any) => d.isVirtual ? "#000" : "#fff")
      .attr("opacity", 0.6);
    ...
    node
      .on("mouseover", (event, d: any) => {
          ...
          node.transition().duration(200).attr("opacity", (n: any) => neighbors.has(n.id) ? 1 : 0.1);
          link.transition().duration(200).attr("stroke-opacity", (l: any) => (l.source.id === d.id || l.target.id === d.id) ? 0.8 : 0.05)
              .attr("stroke", (l: any) => (l.source.id === d.id || l.target.id === d.id) ? "#fff" : "#3f3f46");
      })
      .on("mouseout", () => {
          setHoveredNode(null);
          setHoveredNeighbors(new Set());
          
          node.transition().duration(200).attr("opacity", (d: any) => d.lifecycleStatus === 'dead' && viewMode === 'lifecycle' ? 0.3 : 1);
    ...
    node.append("text")
      .text((d: any) => d.name)
      .attr("x", (d: any) => d.kind === 'directory' ? 15 : 10)
      .attr("y", 4)
      .attr("font-size", (d: any) => d.kind === 'directory' ? "10px" : "8px")
      .attr("font-family", "monospace")
      .attr("fill", (d: any) => d.isVirtual ? "#a855f7" : (d.lifecycleStatus === 'dead' && viewMode === 'lifecycle' ? '#52525b' : '#e4e4e7'))
        ...
        <div className="absolute bottom-4 right-4 pointer-events-none">
            <div className="bg-black/60 backdrop-blur px-3 py-1.5 rounded-full border border-white/10 text-[9px] text-text-muted font-mono uppercase tracking-widest flex items-center gap-2">
                <span className={`w-1.5 h-1.5 rounded-full ${
                    viewMode === 'heatmap' ? 'bg-orange-500' :
