                ...
                {service.status !== 'offline' && (
                  <>
                      ...
                      {service.metrics.network && (
                        <div className="bg-zinc-900/50 border border-zinc-800 p-2 rounded-lg">
                           ...
                           <div className="text-sm font-mono text-white">
                             {service.metrics.network.rx[service.metrics.network.rx.length - 1]} / {service.metrics.network.tx[service.metrics.network.tx.length - 1]} KB/s
                           </div>
                        </div>
                      )}
                      {service.metrics.requests && (
                         <div className="bg-zinc-900/50 border border-zinc-800 p-2 rounded-lg">
                           ...
                           <div className="text-lg font-mono text-white">
                             {service.metrics.requests[service.metrics.requests.length - 1]}
                           </div>
                         </div>
                      )}
                    </div>
                    ...
                    {service.alerts && service.alerts.length > 0 && (
                      <div className="bg-zinc-900/50 border border-zinc-800 p-2 rounded-lg mt-2">
                        <div className="flex items-center space-x-1.5 text-[10px] text-zinc-500 mb-1 font-bold uppercase">
                        ...
                        <div className="space-y-1">
                          {service.alerts.map((alert) => (
                            <div key={alert.id} className={`text-[10px] p-1 rounded ${alert.type === 'error' ? 'bg-red-500/10 text-red-400' : 'bg-yellow-500/10 text-yellow-400'}`}>
