...
export function isUnstableAvatar(value = '') {
    ...
    return /picsum\.photos\/(?:\d|$)/i.test(url) && !/picsum\.photos\/seed\//i.test(url);
}
...
export function stableAvatar(input = {}) {
    const existing = String(input.avatar || input.image || '').trim();
    if (existing && !isUnstableAvatar(existing)) return existing;

    const key = String(input.id || input.userId || input.handle || input.name || 'direct');
    const name = String(input.name || input.title || input.userName || key);
    const [from, to] = PALETTE[stableHash(key) % PALETTE.length];
